...
`;w.div`
  ...
  width: 40vw;
  height: 40vh;
`;const ye=50,Ye=24,tt=16,Xe=40,nt=Xe+tt+Ye,rt=84,de=56,Ee=196,_e=12,Ce=50,ot=24,mt=16,qt=40,is=ot+mt+qt,or=24,sr=52,Pe={topRow:`600 14px ${Ae}`,middleRow:`400 10px ${Ae}`,bottomRow:{name:`600 14px ${Ae}`,number:`600 10px ${Ae}`,hoursInDay:`400 9px ${Ae}`}},st=3,as=1.6,cs=4.5,Jt=12,_t=24,ls="reactSchedulerCanvasHeaderWrapper",ir="reactSchedulerCanvasWrapper",Ie=gt,ds=4,Qt=48,Ze=5,us=40,ar=8,Rt=Ye/2+2,cr=tt/2+Ye+1,lr=2,we=60,De=21,dr=58;var Ue={},fs={get exports(){return Ue},set exports(e){Ue=e}};(function(e,n){(function(t,r){e.exports=r()})(Se,function(){var t=1e3,r=6e4,o=36e5,s="millisecond",i="second",l="minute",u="hour",c="day",d="week",h="month",y="quarter",S="year",x="date",k="Invalid Date",g=/^(\d{4})[-/]?(\d{1,2})?[-/]?(\d{0,2})[Tt\s]*(\d{1,2})?:?(\d{1,2})?:?(\d{1,2})?[.:]?(\d+)?$/,F=/\[([^\]]+)]|Y{1,4}|M{1,4}|D{1,2}|d{1,4}|H{1,2}|h{1,2}|a|A|m{1,2}|s{1,2}|Z{1,2}|SSS/g,X={name:"en",weekdays:"Sunday_Monday_Tuesday_Wednesday_Thursday_Friday_Saturday".split("_"),months:"January_February_March_April_May_June_July_August_September_October_November_December".split("_"),ordinal:function($){var A=["th","st","nd","rd"],T=$%100;return"["+$+(A[(T-20)%10]||A[T]||A[0])+"]"}},Z=function($,A,T){var j=String($);return!j||j.length>=A?$:""+Array(A+1-j.length).join(T)+$},E={s:Z,z:function($){var A=-$.utcOffset(),T=Math.abs(A),j=Math.floor(T/60),Y=T%60;return(A<=0?"+":"-")+Z(j,2,"0")+":"+Z(Y,2,"0")},m:function $(A,T){if(A.date()<T.date())return-$(T,A);var j=12*(T.year()-A.year())+(T.month()-A.month()),Y=A.clone().add(j,h),J=T-Y<0,ee=A.clone().add(j+(J?-1:1),h);return+(-(j+(T-Y)/(J?Y-ee:ee-Y))||0)},a:function($){return $<0?Math.ceil($)||0:Math.floor($)},p:function($){return{M:h,y:S,w:d,d:c,D:x,h:u,m:l,s:i,ms:s,Q:y}[$]||String($||"").toLowerCase().replace(/s$/,"")},u:function($){return $===void 0}},f="en",m={};m[f]=X;var b=function($){return $ instanceof V},_=function $(A,T,j){var Y;if(!A)return f;if(typeof A=="string"){var J=A.toLowerCase();m[J]&&(Y=J),T&&(m[J]=T,Y=J);var ee=A.split("-");if(!Y&&ee.length>1)return $(ee[0])}else{var se=A.name;m[se]=A,Y=se}return!j&&Y&&(f=Y),Y||!j&&f},P=function($,A){if(b($))return $.clone();var T=typeof A=="object"?A:{};return T.date=$,T.args=arguments,new V(T)},z=E;z.l=_,z.i=b,z.w=function($,A){return P($,{locale:A.$L,utc:A.$u,x:A.$x,$offset:A.$offset})};var V=function(){function $(T){this.$L=_(T.locale,null,!0),this.parse(T)}var A=$.prototype;return A.parse=function(T){this.$d=function(j){var Y=j.date,J=j.utc;if(Y===null)return new Date(NaN);if(z.u(Y))return new Date;if(Y instanceof Date)return new Date(Y);if(typeof Y=="string"&&!/Z$/i.test(Y)){var ee=Y.match(g);if(ee){var se=ee[2]-1||0,oe=(ee[7]||"0").substring(0,3);return J?new Date(Date.UTC(ee[1],se,ee[3]||1,ee[4]||0,ee[5]||0,ee[6]||0,oe)):new Date(ee[1],se,ee[3]||1,ee[4]||0,ee[5]||0,ee[6]||0,oe)}}return new Date(Y)}(T),this.$x=T.x||{},this.init()},A.init=function(){var T=this.$d;this.$y=T.getFullYear(),this.$M=T.getMonth(),this.$D=T.getDate(),this.$W=T.getDay(),this.$H=T.getHours(),this.$m=T.getMinutes(),this.$s=T.getSeconds(),this.$ms=T.getMilliseconds()},A.$utils=function(){return z},A.isValid=function(){return this.$d.toString()!==k},A.isSame=function(T,j){var Y=P(T);return this.startOf(j)<=Y&&Y<=this.endOf(j)},A.isAfter=function(T,j){return P(T)<this.startOf(j)},A.isBefore=function(T,j){return this.endOf(j)<P(T)},A.$g=function(T,j,Y){return z.u(T)?this[j]:this.set(Y,T)},A.unix=function(){return Math.floor(this.valueOf()/1e3)},A.valueOf=function(){return this.$d.getTime()},A.startOf=function(T,j){var Y=this,J=!!z.u(j)||j,ee=z.p(T),se=function(v,L){var H=z.w(Y.$u?Date.UTC(Y.$y,L,v):new Date(Y.$y,L,v),Y);return J?H:H.endOf(c)},oe=function(v,L){return z.w(Y.toDate()[v].apply(Y.toDate("s"),(J?[0,0,0,0]:[23,59,59,999]).slice(L)),Y)},B=this.$W,G=this.$M,K=this.$D,U="set"+(this.$u?"UTC":"");switch(ee){case S:return J?se(1,0):se(31,11);case h:return J?se(1,G):se(0,G+1);case d:var C=this.$locale().weekStart||0,O=(B<C?B+7:B)-C;return se(J?K-O:K+(6-O),G);case c:case x:return oe(U+"Hours",0);case u:return oe(U+"Minutes",1);case l:return oe(U+"Seconds",2);case i:return oe(U+"Milliseconds",3);default:return this.clone()}},A.endOf=function(T){return this.startOf(T,!1)},A.$set=function(T,j){var Y,J=z.p(T),ee="set"+(this.$u?"UTC":""),se=(Y={},Y[c]=ee+"Date",Y[x]=ee+"Date",Y[h]=ee+"Month",Y[S]=ee+"FullYear",Y[u]=ee+"Hours",Y[l]=ee+"Minutes",Y[i]=ee+"Seconds",Y[s]=ee+"Milliseconds",Y)[J],oe=J===c?this.$D+(j-this.$W):j;if(J===h||J===S){var B=this.clone().set(x,1);B.$d[se](oe),B.init(),this.$d=B.set(x,Math.min(this.$D,B.daysInMonth())).$d}else se&&this.$d[se](oe);return this.init(),this},A.set=function(T,j){return this.clone().$set(T,j)},A.get=function(T){return this[z.p(T)]()},A.add=function(T,j){var Y,J=this;T=Number(T);var ee=z.p(j),se=function(G){var K=P(J);return z.w(K.date(K.date()+Math.round(G*T)),J)};if(ee===h)return this.set(h,this.$M+T);if(ee===S)return this.set(S,this.$y+T);if(ee===c)return se(1);if(ee===d)return se(7);var oe=(Y={},Y[l]=r,Y[u]=o,Y[i]=t,Y)[ee]||1,B=this.$d.getTime()+T*oe;return z.w(B,this)},A.subtract=function(T,j){return this.add(-1*T,j)},A.format=function(T){var j=this,Y=this.$locale();if(!this.isValid())return Y.invalidDate||k;var J=T||"YYYY-MM-DDTHH:mm:ssZ",ee=z.z(this),se=this.$H,oe=this.$m,B=this.$M,G=Y.weekdays,K=Y.months,U=function(L,H,I,M){return L&&(L[H]||L(j,J))||I[H].slice(0,M)},C=function(L){return z.s(se%12||12,L,"0")},O=Y.meridiem||function(L,H,I){var M=L<12?"AM":"PM";return I?M.toLowerCase():M},v={YY:String(this.$y).slice(-2),YYYY:this.$y,M:B+1,MM:z.s(B+1,2,"0"),MMM:U(Y.monthsShort,B,K,3),MMMM:U(K,B),D:this.$D,DD:z.s(this.$D,2,"0"),d:String(this.$W),dd:U(Y.weekdaysMin,this.$W,G,2),ddd:U(Y.weekdaysShort,this.$W,G,3),dddd:G[this.$W],H:String(se),HH:z.s(se,2,"0"),h:C(1),hh:C(2),a:O(se,oe,!0),A:O(se,oe,!1),m:String(oe),mm:z.s(oe,2,"0"),s:String(this.$s),ss:z.s(this.$s,2,"0"),SSS:z.s(this.$ms,3,"0"),Z:ee};return J.replace(F,function(L,H){return H||v[L]||ee.replace(":","")})},A.utcOffset=function(){return 15*-Math.round(this.$d.getTimezoneOffset()/15)},A.diff=function(T,j,Y){var J,ee=z.p(j),se=P(T),oe=(se.utcOffset()-this.utcOffset())*r,B=this-se,G=z.m(this,se);return G=(J={},J[S]=G/12,J[h]=G,J[y]=G/3,J[d]=(B-oe)/6048e5,J[c]=(B-oe)/864e5,J[u]=B/o,J[l]=B/r,J[i]=B/t,J)[ee]||B,Y?G:z.a(G)},A.daysInMonth=function(){return this.endOf(h).$D},A.$locale=function(){return m[this.$L]},A.locale=function(T,j){if(!T)return this.$L;var Y=this.clone(),J=_(T,j,!0);return J&&(Y.$L=J),Y},A.clone=function(){return z.w(this.$d,this)},A.toDate=function(){return new Date(this.valueOf())},A.toJSON=function(){return this.isValid()?this.toISOString():null},A.toISOString=function(){return this.$d.toISOString()},A.toString=function(){return this.$d.toUTCString()},$}(),R=V.prototype;return P.prototype=R,[["$ms",s],["$s",i],["$m",l],["$H",u],["$W",c],["$M",h],["$y",S],["$D",x]].forEach(function($){R[$[1]]=function(A){return this.$g(A,$[0],$[1])}}),P.extend=function($,A){return $.$i||($(A,V,P),$.$i=!0),P},P.locale=_,P.isDayjs=b,P.unix=function($){return P(1e3*$)},P.en=m[f],P.Ls=m,P.p={},P})})(fs);const D=Ue,ur=e=>e%4===0&&e%100>0||e%400===0?366:365,en=e=>{const n=e.day();return n!==0&&n!==6},fr=(e,n)=>D(`${e.year}-${e.month+1}-${e.dayOfMonth}`).add(n,"months").daysInMonth(),hr=e=>({hour:e.hour(),dayName:e.format("ddd"),dayOfMonth:e.date(),weekOfYear:e.isoWeek(),month:e.month(),monthName:e.format("MMMM"),isBusinessDay:en(e),isCurrentDay:e.isSame(D(),"day"),year:parseInt(e.format("YYYY"))});let Tt=null;const hs=e=>{if(Tt)return Tt;const n=document.createElement("canvas");n.width=12,n.height=12;const t=n.getContext("2d");return t?(t.strokeStyle="rgba(120, 137, 127, 0.045)",t.lineWidth=6,t.beginPath(),t.moveTo(-3,15),t.lineTo(15,-3),t.stroke(),Tt=e.createPattern(n,"repeat"),Tt):null},tn=(e,n,t,r,o,s,i,l=!1)=>{if(e.strokeStyle=i.colors.border,s?e.fillStyle=i.colors.currentDay:o?e.fillStyle="transparent":e.fillStyle=i.colors.primary,e.beginPath(),e.setLineDash([]),e.fillRect(n,t,r,de),l&&!s){const u=hs(e);u&&(e.fillStyle=u,e.fillRect(n,t,r,de))}e.strokeRect(n+.5,t+.5,r,de)},nn=(e,n)=>{let t=0;for(const r of n)r<=e&&t++;return t*De},ps=(e,n,t,r,o,s=[])=>{for(let i=0;i<n;i++){const l=nn(i,s);for(let u=0;u<=t;u++){const c=D(`${r.year}-${r.month+1}-${r.dayOfMonth}`).add(u,"days"),d=c.isSame(D(),"day"),h=c.isBefore(D(),"day");tn(e,u*ye,i*de+l,ye,en(c),d,o,h)}}},gs=(e,n,t,r)=>{e.setLineDash([5,5]),e.strokeStyle=r.colors.border,e.moveTo(n+.5,.5),e.lineTo(n+.5,t+.5),e.stroke()},ms=(e,n,t,r,o,s=[])=>{let i=0,l=-(r.dayOfMonth-1)*_e;const u=n*de+s.length*De;for(let c=0;c<=t;c++){const h=D(`${r.year}-${r.month+1}-${r.dayOfMonth}`).add(c,"weeks").isSame(D(),"week");for(let y=0;y<n;y++){const S=nn(y,s);tn(e,i,y*de+S,rt,!0,h,o)}i+=rt}for(let c=0;c<t;c++){const d=fr(r,c)*_e;gs(e,l,u,o),l+=d}},ys=(e,n,t,r,o,s=[])=>{const i=D(`${r.year}-${r.month+1}-${r.dayOfMonth+1}`);for(let l=0;l<n;l++){const u=nn(l,s);for(let c=0;c<=t;c++){let d;c===Math.floor(t/2)?d=D():c>Math.floor(t/2)?d=D().add(c-Math.floor(t/2),"hours"):d=D().subtract(Math.floor(t/2)-l,"hours");const h=i.isSame(D(),"day")&&d.isSame(D(),"hour");tn(e,c*Ce+Ce/2-.5,l*de+u,Ce,en(d),h,o)}}},vs=(e,n,t,r)=>{const o=t*de+n*De,s=e.canvas.width,i=o+De/2;e.fillStyle=r.mode==="dark"?r.colors.primary+"80":r.colors.primary,e.fillRect(0,o,s,De),e.strokeStyle=r.colors.border,e.lineWidth=1,e.setLineDash([]),e.beginPath(),e.moveTo(0,i+.5),e.lineTo(s,i+.5),e.stroke()},xs=(e,n,t,r,o,s,i=[])=>{if(e.clearRect(0,0,e.canvas.width,e.canvas.height),!!document.getElementById(ir)){switch(n){case 0:ms(e,t,r,o,s,i);break;case 1:ps(e,t,r,o,s,i);break;case 2:ys(e,t,r,o,s,i);break}for(let u=0;u<i.length;u++)vs(e,u,i[u],s)}};var rn={},bs={get exports(){return rn},set exports(e){rn=e}};(function(e,n){(function(t,r){e.exports=r()})(Se,function(){var t="week",r="year";return function(o,s,i){var l=s.prototype;l.week=function(u){if(u===void 0&&(u=null),u!==null)return this.add(7*(u-this.week()),"day");var c=this.$locale().yearStart||1;if(this.month()===11&&this.date()>25){var d=i(this).startOf(r).add(1,r).date(c),h=i(this).endOf(t);if(d.isBefore(h))return 1}var y=i(this).startOf(r).date(c).startOf(t).subtract(1,"millisecond"),S=this.diff(y,t,!0);return S<0?i(this).startOf("week").week():Math.ceil(S)},l.weeks=function(u){return u===void 0&&(u=null),this.week(u)}}})})(bs);const ws=rn;var on={},Ss={get exports(){return on},set exports(e){on=e}};(function(e,n){(function(t,r){e.exports=r()})(Se,function(){return function(t,r,o){r.prototype.dayOfYear=function(s){var i=Math.round((o(this).startOf("day")-o(this).startOf("year"))/864e5)+1;return s==null?i:this.add(s-i,"day")}}})})(Ss);const Cs=on;var sn={},Ms={get exports(){return sn},set exports(e){sn=e}};(function(e,n){(function(t,r){e.exports=r()})(Se,function(){var t="day";return function(r,o,s){var i=function(c){return c.add(4-c.isoWeekday(),t)},l=o.prototype;l.isoWeekYear=function(){return i(this).year()},l.isoWeek=function(c){if(!this.$utils().u(c))return this.add(7*(c-this.isoWeek()),t);var d,h,y,S,x=i(this),k=(d=this.isoWeekYear(),h=this.$u,y=(h?s.utc:s)().year(d).startOf("year"),S=4-y.isoWeekday(),y.isoWeekday()>4&&(S+=7),y.add(S,t));return x.diff(k,"week")+1},l.isoWeekday=function(c){return this.$utils().u(c)?this.day()||7:this.day(this.day()%7?c:c-7)};var u=l.startOf;l.startOf=function(c,d){var h=this.$utils(),y=!!h.u(d)||d;return h.p(c)==="isoweek"?y?this.date(this.date()-(this.isoWeekday()-1)).startOf("day"):this.date(this.date()-1-(this.isoWeekday()-1)+7).endOf("day"):u.bind(this)(c,d)}}})})(Ms);const ks=sn;var an={},$s={get exports(){return an},set exports(e){an=e}};(function(e,n){(function(t,r){e.exports=r()})(Se,function(){return function(t,r,o){r.prototype.isBetween=function(s,i,l,u){var c=o(s),d=o(i),h=(u=u||"()")[0]==="(",y=u[1]===")";return(h?this.isAfter(c,l):!this.isBefore(c,l))&&(y?this.isBefore(d,l):!this.isAfter(d,l))||(h?this.isBefore(c,l):!this.isAfter(c,l))&&(y?this.isAfter(d,l):!this.isBefore(d,l))}}})})($s);const Ds=an;var cn={},Es={get exports(){return cn},set exports(e){cn=e}};(function(e,n){(function(t,r){e.exports=r()})(Se,function(){var t,r,o=1e3,s=6e4,i=36e5,l=864e5,u=/\[([^\]]+)]|Y{1,4}|M{1,4}|D{1,2}|d{1,4}|H{1,2}|h{1,2}|a|A|m{1,2}|s{1,2}|Z{1,2}|SSS/g,c=31536e6,d=2592e6,h=/^(-|\+)?P(?:([-+]?[0-9,.]*)Y)?(?:([-+]?[0-9,.]*)M)?(?:([-+]?[0-9,.]*)W)?(?:([-+]?[0-9,.]*)D)?(?:T(?:([-+]?[0-9,.]*)H)?(?:([-+]?[0-9,.]*)M)?(?:([-+]?[0-9,.]*)S)?)?$/,y={years:c,months:d,days:l,hours:i,minutes:s,seconds:o,milliseconds:1,weeks:6048e5},S=function(f){return f instanceof E},x=function(f,m,b){return new E(f,b,m.$l)},k=function(f){return r.p(f)+"s"},g=function(f){return f<0},F=function(f){return g(f)?Math.ceil(f):Math.floor(f)},X=function(f){return Math.abs(f)},Z=function(f,m){return f?g(f)?{negative:!0,format:""+X(f)+m}:{negative:!1,format:""+f+m}:{negative:!1,format:""}},E=function(){function f(b,_,P){var z=this;if(this.$d={},this.$l=P,b===void 0&&(this.$ms=0,this.parseFromMilliseconds()),_)return x(b*y[k(_)],this);if(typeof b=="number")return this.$ms=b,this.parseFromMilliseconds(),this;if(typeof b=="object")return Object.keys(b).forEach(function($){z.$d[k($)]=b[$]}),this.calMilliseconds(),this;if(typeof b=="string"){var V=b.match(h);if(V){var R=V.slice(2).map(function($){return $!=null?Number($):0});return this.$d.years=R[0],this.$d.months=R[1],this.$d.weeks=R[2],this.$d.days=R[3],this.$d.hours=R[4],this.$d.minutes=R[5],this.$d.seconds=R[6],this.calMilliseconds(),this}}return this}var m=f.prototype;return m.calMilliseconds=function(){var b=this;this.$ms=Object.keys(this.$d).reduce(function(_,P){return _+(b.$d[P]||0)*y[P]},0)},m.parseFromMilliseconds=function(){var b=this.$ms;this.$d.years=F(b/c),b%=c,this.$d.months=F(b/d),b%=d,this.$d.days=F(b/l),b%=l,this.$d.hours=F(b/i),b%=i,this.$d.minutes=F(b/s),b%=s,this.$d.seconds=F(b/o),b%=o,this.$d.milliseconds=b},m.toISOString=function(){var b=Z(this.$d.years,"Y"),_=Z(this.$d.months,"M"),P=+this.$d.days||0;this.$d.weeks&&(P+=7*this.$d.weeks);var z=Z(P,"D"),V=Z(this.$d.hours,"H"),R=Z(this.$d.minutes,"M"),$=this.$d.seconds||0;this.$d.milliseconds&&($+=this.$d.milliseconds/1e3);var A=Z($,"S"),T=b.negative||_.negative||z.negative||V.negative||R.negative||A.negative,j=V.format||R.format||A.format?"T":"",Y=(T?"-":"")+"P"+b.format+_.format+z.format+j+V.format+R.format+A.format;return Y==="P"||Y==="-P"?"P0D":Y},m.toJSON=function(){return this.toISOString()},m.format=function(b){var _=b||"YYYY-MM-DDTHH:mm:ss",P={Y:this.$d.years,YY:r.s(this.$d.years,2,"0"),YYYY:r.s(this.$d.years,4,"0"),M:this.$d.months,MM:r.s(this.$d.months,2,"0"),D:this.$d.days,DD:r.s(this.$d.days,2,"0"),H:this.$d.hours,HH:r.s(this.$d.hours,2,"0"),m:this.$d.minutes,mm:r.s(this.$d.minutes,2,"0"),s:this.$d.seconds,ss:r.s(this.$d.seconds,2,"0"),SSS:r.s(this.$d.milliseconds,3,"0")};return _.replace(u,function(z,V){return V||String(P[z])})},m.as=function(b){return this.$ms/y[k(b)]},m.get=function(b){var _=this.$ms,P=k(b);return P==="milliseconds"?_%=1e3:_=P==="weeks"?F(_/y[P]):this.$d[P],_===0?0:_},m.add=function(b,_,P){var z;return z=_?b*y[k(_)]:S(b)?b.$ms:x(b,this).$ms,x(this.$ms+z*(P?-1:1),this)},m.subtract=function(b,_){return this.add(b,_,!0)},m.locale=function(b){var _=this.clone();return _.$l=b,_},m.clone=function(){return x(this.$ms,this)},m.humanize=function(b){return t().add(this.$ms,"ms").locale(this.$l).fromNow(!b)},m.milliseconds=function(){return this.get("milliseconds")},m.asMilliseconds=function(){return this.as("milliseconds")},m.seconds=function(){return this.get("seconds")},m.asSeconds=function(){return this.as("seconds")},m.minutes=function(){return this.get("minutes")},m.asMinutes=function(){return this.as("minutes")},m.hours=function(){return this.get("hours")},m.asHours=function(){return this.as("hours")},m.days=function(){return this.get("days")},m.asDays=function(){return this.as("days")},m.weeks=function(){return this.get("weeks")},m.asWeeks=function(){return this.as("weeks")},m.months=function(){return this.get("months")},m.asMonths=function(){return this.as("months")},m.years=function(){return this.get("years")},m.asYears=function(){return this.as("years")},f}();return function(f,m,b){t=b,r=b().$utils(),b.duration=function(z,V){var R=b.locale();return x(z,{$l:R},V)},b.isDuration=S;var _=m.prototype.add,P=m.prototype.subtract;m.prototype.add=function(z,V){return S(z)&&(z=z.asMilliseconds()),_.bind(this)(z,V)},m.prototype.subtract=function(z,V){return S(z)&&(z=z.asMilliseconds()),P.bind(this)(z,V)}}})})(Es);const _s=cn;var Ts="Expected a function",pr=0/0,As="[object Symbol]",Ps=/^\s+|\s+$/g,Os=/^[-+]0x[0-9a-f]+$/i,Ys=/^0b[01]+$/i,Is=/^0o[0-7]+$/i,Ls=parseInt,Ns=typeof Se=="object"&&Se&&Se.Object===Object&&Se,Fs=typeof self=="object"&&self&&self.Object===Object&&self,Hs=Ns||Fs||Function("return this")(),Ws=Object.prototype,zs=Ws.toString,Bs=Math.max,js=Math.min,ln=function(){return Hs.Date.now()};function Zs(e,n,t){var r,o,s,i,l,u,c=0,d=!1,h=!1,y=!0;if(typeof e!="function")throw new TypeError(Ts);n=gr(n)||0,dn(t)&&(d=!!t.leading,h="maxWait"in t,s=h?Bs(gr(t.maxWait)||0,n):s,y="trailing"in t?!!t.trailing:y);function S(m){var b=r,_=o;return r=o=void 0,c=m,i=e.apply(_,b),i}function x(m){return c=m,l=setTimeout(F,n),d?S(m):i}function k(m){var b=m-u,_=m-c,P=n-b;return h?js(P,s-_):P}function g(m){var b=m-u,_=m-c;return u===void 0||b>=n||b<0||h&&_>=s}function F(){var m=ln();if(g(m))return X(m);l=setTimeout(F,k(m))}function X(m){return l=void 0,y&&r?S(m):(r=o=void 0,i)}function Z(){l!==void 0&&clearTimeout(l),c=0,r=u=o=l=void 0}function E(){return l===void 0?i:X(ln())}function f(){var m=ln(),b=g(m);if(r=arguments,o=this,u=m,b){if(l===void 0)return x(u);if(h)return l=setTimeout(F,n),S(u)}return l===void 0&&(l=setTimeout(F,n)),i}return f.cancel=Z,f.flush=E,f}function dn(e){var n=typeof e;return!!e&&(n=="object"||n=="function")}function Vs(e){return!!e&&typeof e=="object"}function Gs(e){return typeof e=="symbol"||Vs(e)&&zs.call(e)==As}function gr(e){if(typeof e=="number")return e;if(Gs(e))return pr;if(dn(e)){var n=typeof e.valueOf=="function"?e.valueOf():e;e=dn(n)?n+"":n}if(typeof e!="string")return e===0?e:+e;e=e.replace(Ps,"");var t=Ys.test(e);return t||Is.test(e)?Ls(e.slice(2),t?2:8):Os.test(e)?pr:+e}var it=Zs;const At=[0,1,2];var yt=(e=>(e[e.Tour=0]="Tour",e[e.Transfer=1]="Transfer",e))(yt||{});const mr=e=>At.includes(e),at=e=>{var r;const t=(((r=document.getElementById(Ie))==null?void 0:r.clientWidth)||0)-Ee;switch(e){case 1:return Math.ceil(t/ye)*st;case 2:return Math.ceil(t/Ce)*st;default:return Math.ceil(t/rt)*st}},un=e=>at(e)/st,fn=(e,n)=>{const t=at(n)/2;let r;switch(n){case 1:r=e.subtract(t,"days");break;case 2:r=e.subtract(t,"hours");break;default:r=e.subtract(t,"weeks");break}let o;switch(n){case 1:o=e.add(t,"days");break;case 2:o=e.add(t,"hours");break;default:o=e.add(t,"weeks");break}return{startDate:r,endDate:o}},Xs=(e,n)=>{const t=fn(e,n);return{startDate:t.startDate.toDate(),endDate:t.endDate.toDate()}},hn=()=>{var t;return((((t=document.getElementById(Ie))==null?void 0:t.clientWidth)||0)-Ee)*st},yr=p.createContext({handleGoNext:()=>{},handleScrollNext:()=>{},handleGoPrev:()=>{},handleScrollPrev:()=>{},handleGoToday:()=>{},goToDate:()=>{},zoomIn:()=>{},zoomOut:()=>{},setZoom:()=>{},toggleDisplayActiveUnits:()=>{},updateTilesCoords:()=>{},tilesCoords:[],zoom:0,isNextZoom:!1,isPrevZoom:!1,date:D(),isLoading:!1,cols:0,startDate:{hour:0,dayName:"",dayOfMonth:0,weekOfYear:0,month:0,monthName:"",isCurrentDay:!1,isBusinessDay:!1,year:0},dayOfYear:0,recordsThreshold:0,config:{zoom:0}});D.extend(ws),D.extend(Cs),D.extend(ks),D.extend(Ds),D.extend(_s);const Us=({data:e,children:n,isLoading:t,config:r,defaultStartDate:o=D(),onRangeChange:s,handleToggleDisplayActiveUnits:i,onClearFilterData:l})=>{const{zoom:u,maxRecordsPerPage:c=50}=r,[d,h]=p.useState(u),[y,S]=p.useState(D()),[x,k]=p.useState(!1),[g,F]=p.useState(at(d)),X=At[d]!==At[At.length-1],Z=d!==0,E=p.useMemo(()=>Xs(y,d),[y,d]),f=fn(y,d).startDate,m=D(f).dayOfYear(),b=hr(f),_=p.useRef(null),[P,z]=p.useState([{x:0,y:0}]),V=p.useCallback((U,C="auto")=>{var v,L,H,I;const O=hn();switch(U){case"back":return(v=_.current)==null?void 0:v.scrollTo({behavior:C,left:O/3});case"forward":return(L=_.current)==null?void 0:L.scrollTo({behavior:C,left:O/3});case"middle":{const M=O/st/4;return(H=_.current)==null?void 0:H.scrollTo({behavior:C,left:O/2-M})}default:return(I=_.current)==null?void 0:I.scrollTo({behavior:C,left:O/2})}},[]),R=U=>{z(U)},$=p.useCallback(U=>{const C=un(d);let O;switch(d){case 0:O=C*7;break;case 1:O=C;break;case 2:O=Math.ceil(C/_t);break}it(()=>{switch(U){case"back":S(L=>L.subtract(O,"days"));break;case"forward":S(L=>L.add(O,"days"));break;case"middle":S(D());break}s==null||s(E)},300)()},[s,E,d]);p.useEffect(()=>{_.current=document.getElementById(Ie),F(at(d))},[d]),p.useEffect(()=>{const U=()=>F(at(d));return window.addEventListener("resize",U),()=>window.removeEventListener("resize",U)},[d]),p.useEffect(()=>{s==null||s(E)},[s,E]),p.useEffect(()=>{k(!1)},[o]),p.useEffect(()=>{x||(V("middle"),k(!0),S(o))},[o,x,V]);const A=()=>{t||(S(U=>d===2?U.add(or,"hours"):U.add(lr,"weeks")),s==null||s(E))},T=p.useCallback(()=>{t||($("forward"),it(()=>{V("forward")},500)())},[t,$,V]),j=()=>{t||(S(U=>d===2?U.subtract(or,"hours"):U.subtract(lr,"weeks")),s==null||s(E))},Y=p.useCallback(()=>{!x||t||($("back"),it(()=>{V("back")},500)())},[x,t,$,V]),J=p.useCallback(()=>{t||($("middle"),it(()=>{V("middle","smooth")},300)())},[t,$,V]),ee=p.useCallback(U=>{if(t)return;const C=D(U).startOf("day");C.isValid()&&(S(C),s==null||s(E),setTimeout(()=>{V("middle","smooth")},300))},[t,V,s,E]),se=()=>B(d+1),oe=()=>B(d-1),B=U=>{mr(U)&&(h(U),F(at(U)),s==null||s(E))},G=()=>i==null?void 0:i(),{Provider:K}=yr;return a.jsx(K,{value:{data:e,config:r,handleGoNext:A,handleScrollNext:T,handleGoPrev:j,handleScrollPrev:Y,handleGoToday:J,goToDate:ee,zoomIn:se,zoomOut:oe,setZoom:B,zoom:d,isNextZoom:X,isPrevZoom:Z,date:y,isLoading:t,cols:g,startDate:b,dayOfYear:m,toggleDisplayActiveUnits:G,tilesCoords:P,updateTilesCoords:R,recordsThreshold:c,onClearFilterData:l},children:n})},Le=()=>p.useContext(yr),vr=(e,n,t)=>{e.canvas.width=n*window.devicePixelRatio,e.canvas.height=t*window.devicePixelRatio,e.canvas.style.width=n+"px",e.canvas.style.height=t+"px",e.scale(window.devicePixelRatio,window.devicePixelRatio)},xr=(e,n)=>{if(n.length===0)return e;let t=e,r=0;for(const o of n){const s=o*de+r*De;if(e>=s+De)r++;else if(e>=s)return o*de+r*De-r*De}return t-r*De},Ks=5,br=(e,n)=>{const t=Math.abs(n.x-e.x),r=Math.abs(n.y-e.y);return Math.sqrt(t*t+r*r)>Ks},vt=(e,n,t)=>{const r=t.getBoundingClientRect();return{x:e-r.left+t.scrollLeft,y:n-r.top+t.scrollTop}},qs=({data:e,baseData:n,zoom:t,startDate:r,onEventDrop:o,onEventDrag:s,draggableConfig:i={},gridRef:l,separatorRowIndices:u=[]})=>{const c=n?n.length>0&&n[0].data.length>0&&!Array.isArray(n[0].data[0])?n.map(O=>({...O,data:[O.data]})):n:e,{enabled:d=!0,isDraggable:h,resourceOnly:y=!1,isValidDrop:S}=i,[x,k]=p.useState("idle"),[g,F]=p.useState(null),[X,Z]=p.useState({x:0,y:0}),[E,f]=p.useState({width:0,height:48}),[m,b]=p.useState(null),[_,P]=p.useState(!0),z=p.useRef({x:0,y:0}),V=p.useRef({x:0,y:0}),R=p.useRef({x:0,y:0}),$=p.useRef(null),A=p.useRef(null),T=p.useRef(0),j=p.useRef(null),Y=p.useCallback(O=>!d||O.draggable===!1?!1:h?h(O):!0,[d,h]),J=p.useCallback((O,v)=>{const L=xr(v,u),H=Math.floor(L/de);let I;switch(t){case 0:I=_e*7;break;case 1:I=ye;break;case 2:I=Ce;break;default:I=ye}const M=Math.floor(O/I);let te;const N=D().year(r.year).month(r.month).date(r.dayOfMonth).hour(r.hour).minute(0).second(0).millisecond(0);switch(t){case 0:te=N.add(M*7,"days").toDate();break;case 1:te=N.add(M,"days").toDate();break;case 2:te=N.add(M,"hours").toDate();break;default:te=N.toDate()}return{snappedDate:te,snappedResourceIndex:H}},[t,r,u]),ee=p.useCallback((O,v,L,H)=>{const I=[],M=v.getTime(),te=L.getTime(),N=c.find(re=>re.id===H);if(!N)return I;const q=[];for(const re of N.data)Array.isArray(re)?q.push(...re):q.push(re);for(const re of q){if(re.segmentId===O.segmentId)continue;const W=re.startDate.getTime(),ae=re.endDate.getTime();if(M>=W&&M<ae||te>W&&te<=ae||M<=W&&te>=ae){const ue=new Date(Math.max(M,W)),me=new Date(Math.min(te,ae)),Me=me.getTime()-ue.getTime();I.push({event:re,conflictStart:ue,conflictEnd:me,overlapDuration:Me})}}return I},[c]),se=p.useCallback((O,v,L,H)=>{const I=[],M=v.getTime(),te=L.getTime(),N=D(v).format("YYYY-MM-DD"),q=c.find(W=>W.id===H);if(!q)return I;const re=[];for(const W of q.data)Array.isArray(W)?re.push(...W):re.push(W);for(const W of re){if(W.segmentId===O.segmentId)continue;const ae=W.startDate.getTime(),le=W.endDate.getTime(),ue=D(W.startDate).format("YYYY-MM-DD"),me=D(W.endDate).format("YYYY-MM-DD"),Me=D(L).format("YYYY-MM-DD");if(!(ue===N||me===N||ue===Me||me===Me||D(W.startDate).isBefore(v,"day")&&D(W.endDate).isAfter(L,"day"))||M>=ae&&M<le||te>ae&&te<=le||M<=ae&&te>=le)continue;let ie,ce;le<=M?(ie=M-le,ce="before"):(ie=ae-te,ce="after"),I.push({event:W,timeGap:ie,position:ce})}return I.sort((W,ae)=>W.timeGap-ae.timeGap)},[c]),oe=p.useCallback((O,v,L)=>{const H=J(v,L);let I,M;if(y)I=O.startDate,M=O.endDate;else{const le=D(O.endDate).diff(O.startDate);I=H.snappedDate,M=D(I).add(le,"milliseconds").toDate()}let te=0,N="",q;for(const le of e){const ue=Math.max(le.data.length,1);if(H.snappedResourceIndex<te+ue){N=le.id,q=le.capacity;break}te+=ue}if(!N)return null;let re=!0;q!==void 0&&O.totalPassengers!==void 0&&(re=O.totalPassengers<=q);const W=ee(O,I,M,N),ae=W.length===0?se(O,I,M,N):[];return{startDate:I,endDate:M,resourceId:N,resourceIndex:H.snappedResourceIndex,resourceCapacity:q,hasCapacity:re,conflicts:W,hasConflict:W.length>0,nearbyEvents:ae}},[J,e,y,ee,se]),B=p.useCallback((O,v)=>{if(!s)return;const L=Date.now();if(L-T.current<100)return;T.current=L;const H={event:O,currentStartDate:v.startDate,currentEndDate:v.endDate,currentResourceId:v.resourceId,conflicts:v.conflicts};s(H)},[s]),G=p.useCallback((O,v)=>{if(!Y(O)||!l.current)return;v.preventDefault(),v.stopPropagation();const L=v.target.closest('[style*="left"]');let H=0,I=0;L&&L.style.left&&L.style.top&&(H=parseInt(L.style.left),I=parseInt(L.style.top));const M=vt(v.clientX,v.clientY,l.current);z.current={x:H,y:I},V.current={x:v.clientX,y:v.clientY},R.current={x:M.x-H,y:20},j.current={startDate:O.startDate,endDate:O.endDate,resourceId:""};for(const q of e){for(const re of q.data)if(re.some(W=>W.segmentId===O.segmentId)){j.current.resourceId=q.id;break}if(j.current.resourceId)break}F(O),k("potential"),Z({x:H,y:I});let te=100,N=48;if(L){const q=L.getBoundingClientRect();te=q.width,N=q.height}f({width:te,height:N})},[Y,l,e,t]),K=p.useCallback(O=>{if(!l.current)return;let v=l.current;for(;v&&v!==document.body;){const W=window.getComputedStyle(v);if(v.scrollHeight>v.clientHeight&&(W.overflowY==="auto"||W.overflowY==="scroll"||W.overflow==="auto"||W.overflow==="scroll"))break;v=v.parentElement}(!v||v===document.body)&&(v=document.documentElement);const L=v.getBoundingClientRect(),H=O.clientY,I=50,M=5,te=H-L.top,N=L.bottom-H;let q=!1,re=0;te<I&&te>0?(q=!0,re=-M*(1-te/I)):N<I&&N>0&&(q=!0,re=M*(1-N/I)),q?(A.current&&cancelAnimationFrame(A.current),A.current=requestAnimationFrame(()=>{v.scrollTop+=re,x==="dragging"&&K(O)})):A.current&&(cancelAnimationFrame(A.current),A.current=null)},[l,x]),U=p.useCallback(O=>{if(x==="idle"||x==="animating"||!g||!l.current)return;const v={x:O.clientX,y:O.clientY};if(x==="potential")if(br(V.current,v))k("dragging");else return;K(O);const L=vt(O.clientX,O.clientY,l.current);$.current&&cancelAnimationFrame($.current),$.current=requestAnimationFrame(()=>{const H={x:L.x-R.current.x,y:L.y-R.current.y};Z(H);const I=oe(g,L.x,L.y);if(I&&S){const M={event:g,currentStartDate:I.startDate,currentEndDate:I.endDate,currentResourceId:I.resourceId,conflicts:I.conflicts};I.hasConflict=!S(M)}if(b(I),I){const M=I.hasCapacity!==!1;P(M),B(g,I)}})},[x,g,l,oe,B,S,K]),C=p.useCallback(async O=>{if(x==="idle"||x==="animating")return;const v={x:O.clientX,y:O.clientY};if(!br(V.current,v)||x==="potential"){k("idle"),F(null),b(null);return}if(!g||!m||!j.current){k("idle"),F(null),b(null);return}if(m.hasCapacity===!1){P(!1),k("animating"),Z(z.current),setTimeout(()=>{k("idle"),F(null),b(null),P(!0)},300);return}const H={event:g,originalStartDate:j.current.startDate,originalEndDate:j.current.endDate,originalResourceId:j.current.resourceId,newStartDate:m.startDate,newEndDate:m.endDate,newResourceId:m.resourceId,hasConflict:m.hasConflict,conflicts:m.conflicts};let I=!0;if(o)try{const M=o(H);I=M instanceof Promise?await M:M}catch{I=!1}I?(P(!0),k("idle"),F(null),b(null)):(P(!1),k("animating"),Z(z.current),setTimeout(()=>{k("idle"),F(null),b(null),P(!0)},300))},[x,g,m,o,S]);return p.useEffect(()=>{if(x==="potential"||x==="dragging"){const O=L=>U(L),v=L=>C(L);return document.addEventListener("mousemove",O),document.addEventListener("mouseup",v),()=>{document.removeEventListener("mousemove",O),document.removeEventListener("mouseup",v)}}else return()=>{}},[x,U,C]),p.useEffect(()=>()=>{$.current&&(cancelAnimationFrame($.current),$.current=null),A.current&&(cancelAnimationFrame(A.current),A.current=null)},[]),p.useEffect(()=>{(x==="idle"||x==="animating")&&($.current&&(cancelAnimationFrame($.current),$.current=null),A.current&&(cancelAnimationFrame(A.current),A.current=null))},[x]),p.useEffect(()=>{(x==="dragging"||x==="potential")&&(x==="dragging"?(k("animating"),Z(z.current),setTimeout(()=>{k("idle"),F(null),b(null)},300)):(k("idle"),F(null),b(null)))},[t]),p.useEffect(()=>{if((x==="dragging"||x==="potential")&&g){let O=!1;for(const v of e){for(const L of v.data)if(L.some(H=>H.segmentId===g.segmentId)){O=!0;break}if(O)break}O||(x==="dragging"?(k("animating"),Z(z.current),setTimeout(()=>{k("idle"),F(null),b(null)},300)):(k("idle"),F(null),b(null)))}},[e,x,g]),{dragState:x,draggedEvent:g,ghostPosition:X,ghostDimensions:E,dropTarget:m,isValidDrop:_,handleDragStart:G,isDraggable:Y,draggingEventId:(g==null?void 0:g.segmentId)||null,resourceOnly:y}},Js=({data:e,baseData:n,zoom:t,startDate:r,onTimeRangeSelect:o,onMultiTimeRangeSelect:s,clickToAddConfig:i={},gridRef:l,isDragging:u,separatorRowIndices:c=[]})=>{const{enabled:d=!1,isSelectable:h}=i,y=d&&!!o,S=p.useCallback(v=>{let L=0;for(const H of c)H<=v&&L++;return v*de+L*De},[c]),[x,k]=p.useState("idle"),[g,F]=p.useState(null),[X,Z]=p.useState(null),[E,f]=p.useState(null),[m,b]=p.useState(!1),[_,P]=p.useState([]),[z,V]=p.useState(!1),R=p.useRef(null),$=p.useRef(null),A=p.useRef(null),T=p.useCallback(()=>{switch(t){case 0:return _e*7;case 1:return ye;case 2:return Ce;default:return ye}},[t]),j=p.useCallback(v=>{const L=T(),H=Math.floor(v/L),I=D().year(r.year).month(r.month).date(r.dayOfMonth).hour(r.hour).minute(0).second(0).millisecond(0);switch(t){case 0:return I.add(H*7,"days").toDate();case 1:return I.add(H,"days").toDate();case 2:return I.add(H,"hours").toDate();default:return I.toDate()}},[t,r,T]),Y=p.useCallback(v=>{const L=xr(v,c),H=Math.floor(L/de);let I=0;for(const M of e){const te=Math.max(M.data.length,1);if(H<I+te)return{resourceId:M.id,resourceIndex:H,resourceLabel:M.label};I+=te}return null},[e,c]),J=p.useCallback(v=>{const L=T();return Math.floor(v/L)*L},[T]),ee=p.useCallback((v,L,H,I=[])=>{const M=[],N=(n||e).find(W=>W.id===v),q=L.getTime(),re=H.getTime();if(N){const W=N.data[0],ae=W&&Array.isArray(W)?N.data.flat():N.data;for(const le of ae){const ue=new Date(le.startDate).getTime(),me=new Date(le.endDate).getTime();if(q<me&&re>ue){const Me=new Date(Math.max(q,ue)),ve=new Date(Math.min(re,me)),Q=ve.getTime()-Me.getTime();M.push({event:le,conflictStart:Me,conflictEnd:ve,overlapDuration:Q})}}}for(const W of I){if(W.resourceId!==v)continue;const ae=W.startDate.getTime(),le=W.endDate.getTime();if(q<le&&re>ae){const ue=new Date(Math.max(q,ae)),me=new Date(Math.min(re,le)),Me=me.getTime()-ue.getTime(),ve={segmentId:`pending-${W.startDate.getTime()}`,reservationId:`pending-${W.startDate.getTime()}`,startDate:W.startDate,endDate:W.endDate,occupancy:0,title:`New Event (${W.resourceLabel.title})`,bookingNumber:"",description:"Pending selection"};M.push({event:ve,conflictStart:ue,conflictEnd:me,overlapDuration:Me})}}return M},[e,n]),se=p.useCallback(v=>{if(!y||u||!l.current||v.button!==0)return;const L=v.target;if(L.closest("[data-segment-id]")||L.closest("[data-multi-select-ui]"))return;const H=vt(v.clientX,v.clientY,l.current),I=Y(H.y);if(!I)return;R.current={x:v.clientX,y:v.clientY},$.current=I.resourceIndex;const M=J(H.x),te=T(),N=S(I.resourceIndex);F(H),Z(H),f({x:M,y:N,width:te,height:de}),k("selecting")},[y,u,l,Y,J,T,S]),oe=p.useCallback(v=>{if(x!=="selecting"||!l.current||$.current===null)return;const L=vt(v.clientX,v.clientY,l.current);A.current&&cancelAnimationFrame(A.current),A.current=requestAnimationFrame(()=>{Z(L);const H=T(),I=J((g==null?void 0:g.x)||0),M=J(L.x),te=S($.current),N=Math.min(I,M),q=Math.max(I,M)+H;f({x:N,y:te,width:q-N,height:de})})},[x,l,g,T,J,S]),B=p.useCallback(v=>{if(x!=="selecting")return;if(!l.current||!g||!R.current){k("idle"),F(null),Z(null),f(null);return}const L=vt(v.clientX,v.clientY,l.current),H=Y(g.y);if(!H){k("idle"),F(null),Z(null),f(null);return}const I=Math.min(g.x,L.x),M=Math.max(g.x,L.x),te=j(I),N=j(M),q=D(N).hour(23).minute(59).second(0).millisecond(0).toDate();if(h&&!h(H.resourceId,te,q)){k("idle"),F(null),Z(null),f(null);return}const re=ee(H.resourceId,te,q,_),W=re.length>0,ae={startDate:te,endDate:q,resourceId:H.resourceId,resourceLabel:H.resourceLabel,zoomLevel:t,hasConflict:W,conflicts:W?re:void 0};if(m)P(le=>[...le,ae]),V(!0);else if(o){const le=o(ae),ue=me=>{me!=null&&me.continueMultiSelect&&(b(!0),P([ae]),V(!0))};le instanceof Promise?le.then(ue):ue(le)}k("idle"),F(null),Z(null),f(null),R.current=null,$.current=null},[x,l,g,Y,j,h,o,t,m,ee,_]),G=p.useCallback(()=>{if(_.length>0&&s){V(!1);const v=s(_),L=H=>{H!=null&&H.continueMultiSelect?V(!0):(P([]),b(!1),V(!1))};v instanceof Promise?v.then(L):L(v);return}P([]),b(!1),V(!1)},[_,s]),K=p.useCallback(()=>{P([]),b(!1),V(!1)},[]),U=p.useCallback(v=>{P(L=>{const H=L.filter((I,M)=>M!==v);return H.length===0&&(b(!1),V(!1)),H})},[]),C=p.useCallback((v,L)=>{P(H=>H.map((I,M)=>{if(M!==v)return I;const te={...I,...L},N=H.filter((re,W)=>W!==v),q=ee(te.resourceId,te.startDate,te.endDate,N);return{...te,hasConflict:q.length>0,conflicts:q.length>0?q:void 0}}))},[ee]),O=p.useCallback(v=>{v.key==="Escape"&&(x==="selecting"?(k("idle"),F(null),Z(null),f(null),R.current=null,$.current=null):m&&_.length>0&&(P([]),b(!1),V(!1)))},[x,m,_.length]);return p.useEffect(()=>{if(x==="selecting")return document.addEventListener("mousemove",oe),document.addEventListener("mouseup",B),document.addEventListener("keydown",O),()=>{document.removeEventListener("mousemove",oe),document.removeEventListener("mouseup",B),document.removeEventListener("keydown",O)}},[x,oe,B,O]),p.useEffect(()=>{if(m&&_.length>0)return document.addEventListener("keydown",O),()=>{document.removeEventListener("keydown",O)}},[m,_.length,O]),p.useEffect(()=>()=>{A.current&&(cancelAnimationFrame(A.current),A.current=null)},[]),p.useEffect(()=>{u&&x==="selecting"&&(k("idle"),F(null),Z(null),f(null),R.current=null,$.current=null)},[u,x]),{selectionState:x,selectionStart:g,selectionEnd:X,selectionBox:E,handleGridMouseDown:se,isEnabled:y,pendingSelections:_,confirmSelections:G,clearSelections:K,removeSelection:U,updateSelection:C,isMultiSelectActive:m,hasUnconfirmedSelections:z}},Qs=w.div`
...
`,Ni=w.div`
  ...
  align-items: center;
`,wn={coords:{x:0,y:0},mouseCoords:{x:0,y:0},resourceIndex:0,disposition:{taken:{hours:0,minutes:0},free:{hours:0,minutes:0},overtime:{hours:0,minutes:0}},reservationData:{startTime:"",startDate:"",client:"",eventName:"",reservationType:yt.Tour,bookingNumber:""},tileBounds:{x:0,y:0,width:0,height:0}};function Fi(e,n){const t=n?[...n].sort((l,u)=>l.maxPassengers-u.maxPassengers):[],r=[];for(const l of t){const u=e.filter(c=>!c.isSubcontract&&c.categoryId===l.id);u.length>0&&r.push({type:"category",category:l,items:u})}const o=t.length>0,s=e.filter(l=>!l.isSubcontract&&(!l.categoryId||!o));s.length>0&&o?r.push({type:"uncategorized",items:s}):s.length>0&&r.push({type:"uncategorized",items:s});const i=e.filter(l=>l.isSubcontract);return i.length>0&&r.push({type:"subcontract",items:i}),r}const Hi=({data:e,baseData:n,categories:t,onTileClick:r,onItemClick:o,toggleTheme:s,topBarWidth:i,onEventDrop:l,onEventDrag:u,draggableConfig:c,onTimeRangeSelect:d,onMultiTimeRangeSelect:h,clickToAddConfig:y})=>{const[S,x]=p.useState(wn),[k,g]=p.useState(e),[F,X]=p.useState(!1),[Z,E]=p.useState(!1),[f,m]=p.useState(""),[b,_]=p.useState(new Set),{zoom:P,startDate:z,isLoading:V,config:{includeTakenHoursOnWeekendsInDayView:R,showTooltip:$,showThemeToggle:A}}=Le(),T=p.useRef(null),j=p.useRef(null),[Y,J]=p.useState(124),{page:ee,projectsPerPerson:se,rowsPerItem:oe,currentPageNum:B,pagesAmount:G,next:K,previous:U,reset:C}=vi(k),{effectiveCategories:O,effectivePage:v}=p.useMemo(()=>{if(t&&t.length>0)return{effectiveCategories:t,effectivePage:ee};const Q=ci(ee);if(Q.categories.length===0)return{effectiveCategories:void 0,effectivePage:ee};const ie=ee.map(ce=>{if(ce.isSubcontract||ce.capacity==null)return ce;const xe=Q.capacityToCategoryId.get(ce.capacity);return xe?{...ce,categoryId:xe}:ce});return{effectiveCategories:Q.categories,effectivePage:ie}},[t,ee]),L=p.useCallback(Q=>{_(ie=>{const ce=new Set(ie);return ce.has(Q)?ce.delete(Q):ce.add(Q),ce})},[]),H=p.useMemo(()=>{const Q=[],ie=O?[...O].sort((ce,xe)=>ce.maxPassengers-xe.maxPassengers):[];for(const ce of ie)v.some(xe=>!xe.isSubcontract&&xe.categoryId===ce.id)&&Q.push(ce.id);return v.some(ce=>ce.isSubcontract)&&Q.push("__subcontract__"),Q},[O,v]),I=p.useCallback(()=>{_(new Set)},[]),M=p.useCallback(()=>{_(new Set(H))},[H]),{visiblePage:te,visibleRowsPerItem:N,visibleTotalRows:q,visibleProjectsPerPerson:re,separatorRowIndices:W}=p.useMemo(()=>{const Q=Fi(v,O),ie=((O==null?void 0:O.length)??0)>0,ce=new Map;ee.forEach((ke,Be)=>ce.set(ke.id,Be));const xe=[],be=[],We=[],ze=[];let dt=0;for(const ke of Q)if(ke.type==="subcontract"||ke.type==="category"&&ie){const ut=ke.type==="subcontract"?"__subcontract__":ke.category.id,ft=b.has(ut);if(ze.push(dt),!ft)for(const Ke of ke.items){const Yt=ce.get(Ke.id)??0,It=oe[Yt];xe.push(Ke),be.push(It),We.push(se[Yt]),dt+=It}}else for(const ut of ke.items){const ft=ce.get(ut.id)??0,Ke=oe[ft];xe.push(ut),be.push(Ke),We.push(se[ft]),dt+=Ke}const En=be.reduce((ke,Be)=>ke+Be,0);return{visiblePage:xe,visibleRowsPerItem:be,visibleTotalRows:En,visibleProjectsPerPerson:We,separatorRowIndices:ze}},[v,O,ee,b,oe,se]),ae=p.useRef(it((Q,ie,ce,xe,be,We)=>{if(!T.current)return;const{tile:ze,segmentId:dt}=me(Q);if(!dt||!ze){X(!1),x(wn);return}const En=ue(dt,ie),ke=T.current.getBoundingClientRect(),Be=ze.getBoundingClientRect(),ut={x:Q.clientX-ke.left,y:Q.clientY-ke.top},ft={x:Q.clientX-ke.left,y:Q.clientY-ke.top},Ke={x:Be.left-ke.left,y:Be.top-ke.top,width:Be.width,height:Be.height},{coords:{x:Yt,y:It},resourceIndex:Sl,disposition:Cl,reservationData:Ml}=ii(En,ce,ut,xe,be,We,R);x({coords:{x:Yt,y:It},mouseCoords:ft,resourceIndex:Sl,disposition:Cl,reservationData:Ml,tileBounds:Ke}),X(!0)},4)),le=p.useRef(it((Q,ie)=>{C(),g(Q.map(ce=>({...ce,data:ce.data.filter(xe=>{const{title:be,description:We,subtitle:ze}=xe;return(be==null?void 0:be.toLowerCase().includes(ie.toLowerCase()))||(ze==null?void 0:ze.toLowerCase().includes(ie.toLowerCase()))||(We==null?void 0:We.toLowerCase().includes(ie.toLowerCase()))})})).filter(ce=>ce.data.length>0))},500)),ue=(Q,ie)=>{if(Q)return ie.flatMap(ce=>ce.data).find(ce=>ce.segmentId===Q)},me=Q=>{if(!Q.target)return{tile:null,segmentId:null};const ie=Q.target.closest("[data-segment-id]");return ie?{tile:ie,segmentId:ie.getAttribute("data-segment-id")}:{tile:null,segmentId:null}},Me=Q=>{const ie=Q.target.value;m(ie),le.current.cancel(),ie?le.current(e,ie):(C(),g(e))},ve=p.useCallback(()=>{ae.current.cancel(),X(!1),x(wn)},[]);return p.useEffect(()=>{const Q=ce=>ae.current(ce,e,z,N,re,P),ie=T.current;if(ie)return ie.addEventListener("mousemove",Q),ie.addEventListener("mouseleave",ve),()=>{ie.removeEventListener("mousemove",Q),ie.removeEventListener("mouseleave",ve)}},[ae,ve,re,N,z,P,e]),p.useEffect(()=>{f?(le.current.cancel(),le.current(e,f)):g(e)},[e,f]),p.useLayoutEffect(()=>{const Q=j.current;if(!Q)return;const ie=()=>J(Q.offsetHeight);ie();const ce=new ResizeObserver(ie);return ce.observe(Q),()=>ce.disconnect()},[]),a.jsxs(Ii,{children:[a.jsx(Na,{headerHeight:Y,data:v,categories:O,pageNum:B,pagesAmount:G,rows:oe,onLoadNext:K,onLoadPrevious:U,searchInputValue:f,onSearchInputChange:Me,onItemClick:o,collapsedGroups:b,onToggleGroup:L,allGroupIds:H,onExpandAll:I,onCollapseAll:M}),a.jsxs(Li,{children:[a.jsx(uc,{ref:j,zoom:P,topBarWidth:i,showThemeToggle:A,toggleTheme:s}),e.length?a.jsx(ti,{data:te,baseData:n||e,zoom:P,rows:q,ref:T,onTileClick:r,onEventDrop:l,onEventDrag:u,draggableConfig:c,onDragStateChange:E,onTimeRangeSelect:d,onMultiTimeRangeSelect:h,clickToAddConfig:y,separatorRowIndices:W}):a.jsx(Ni,{width:i,children:V?a.jsx(Mn,{isLoading:V,position:"left"}):a.jsx(Yi,{})}),$&&F&&!Z&&(S==null?void 0:S.resourceIndex)>-1&&a.jsx(Vc,{tooltipData:S})]})]})},Wi=w.div`
  width: 100%;
  display: grid;
...
`,ca=w.div`
  ...
  z-index: 4;
  pointer-events: none;
`,la=()=>{const{date:e,zoom:n,data:t,goToDate:r}=Le(),o=Ne(),s=p.useRef(null),[i,l]=p.useState(null),u=p.useMemo(()=>Array.from({length:12},(E,f)=>D().month(f).format("MMM").toUpperCase()),[o]),c=e.year(),d=p.useMemo(()=>D(new Date(c,0,1)),[c]),h=p.useMemo(()=>D(new Date(c+1,0,1)).diff(d,"day"),[c,d]),y=E=>E.diff(d,"day")/h*100,S=p.useMemo(()=>{const E=Math.ceil(h/7),f=new Array(E).fill(0),m=new Array(E).fill(0);for(const _ of t??[])for(const P of _.data??[]){const z=D(P.startDate);if(z.year()!==c)continue;const V=Math.floor(z.diff(d,"day")/7);if(V<0||V>=E)continue;f[V]+=1;const R=P.readiness==="sin_chofer"?2:P.readiness==="sin_avisar"?1:0;R>m[V]&&(m[V]=R)}const b=Math.max(1,...f);return f.map((_,P)=>({h:_/b*100,sev:m[P]}))},[t,c,d,h]),x=D(),k=x.year()===c?y(x):null,g=n===1?un(1)/2:n===0?un(0)*7/2:.5,F=Math.max(0,y(e.subtract(g,"day"))),X=Math.min(100,y(e.add(g,"day")))-F,Z=E=>{var b;const f=(b=s.current)==null?void 0:b.getBoundingClientRect();if(!f)return null;const m=Math.min(1,Math.max(0,(E-f.left)/f.width));return{f:m,d:d.add(Math.round(m*h),"day")}};return a.jsxs(Qi,{children:[a.jsxs(Ri,{children:["Navegar",a.jsx("br",{}),"por fecha"]}),a.jsxs(ea,{ref:s,onClick:E=>{const f=Z(E.clientX);f&&r(f.d.toDate())},onMouseMove:E=>{const f=Z(E.clientX);f&&l({left:f.f*100,label:`${f.d.date()} ${u[f.d.month()]}`})},onMouseLeave:()=>l(null),children:[a.jsx(ta,{children:u.map((E,f)=>a.jsx("span",{style:{left:`${y(D(new Date(c,f,1)))}%`},children:E},f))}),u.map((E,f)=>f===0?null:a.jsx(na,{style:{left:`${y(D(new Date(c,f,1)))}%`}},f)),a.jsx(ra,{children:S.map((E,f)=>a.jsx(oa,{$sev:E.sev,style:{height:`${E.h}%`}},f))}),a.jsx(ia,{style:{left:`${F}%`,width:`${X}%`}}),k!==null&&a.jsx(sa,{style:{left:`${k}%`},children:a.jsx("span",{children:"HOY"})}),i&&a.jsxs(a.Fragment,{children:[a.jsx(aa,{style:{left:`${i.left}%`}}),a.jsx(ca,{style:{left:`${i.left}%`},children:i.label})]})]})]})},da=w.div`
  position: absolute;
  top: 0;
...
`,ua=w.div`
  position: relative;
`,fa=({data:e,baseData:n,categories:t,onTileClick:r,topBarWidth:o,onItemClick:s,toggleTheme:i,onEventDrop:l,onEventDrag:u,draggableConfig:c,schedulerRef:d,onTimeRangeSelect:h,onMultiTimeRangeSelect:y,clickToAddConfig:S})=>{const{goToDate:x,handleGoToday:k,zoomIn:g,zoomOut:F,zoom:X}=Le();return p.useImperativeHandle(d,()=>({goToDate:x,goToToday:k,setZoom:Z=>{if(!mr(Z))return;const E=Z-X;if(E>0)for(let f=0;f<E;f++)g();else for(let f=0;f<Math.abs(E);f++)F()}}),[x,k,X,g,F]),a.jsx(Hi,{data:e,baseData:n,categories:t,onTileClick:r,topBarWidth:o,onItemClick:s,toggleTheme:i,onEventDrop:l,onEventDrag:u,draggableConfig:c,onTimeRangeSelect:h,onMultiTimeRangeSelect:y,clickToAddConfig:S})},ha=p.forwardRef(function({data:n,categories:t,baseData:r,config:o,startDate:s,onRangeChange:i,onTileClick:l,handleToggleDisplayActiveUnits:u,onClearFilterData:c,onItemClick:d,isLoading:h,onEventDrop:y,onEventDrag:S,draggableConfig:x,onTimeRangeSelect:k,onMultiTimeRangeSelect:g,clickToAddConfig:F},X){var T;const Z=p.useMemo(()=>({zoom:0,filterButtonState:1,includeTakenHoursOnWeekendsInDayView:!1,showTooltip:!0,showTopbar:!0,showLegend:!0,translations:void 0,...o}),[o]),E=p.useRef(null),f=p.useRef(null),[m,b]=p.useState((T=E.current)==null?void 0:T.clientWidth),_=p.useMemo(()=>D(s),[s]),[P,z]=p.useState(Z.defaultTheme??"light"),V=()=>{z(P==="light"?"dark":"light")},R=P==="light"?os:ss,$=Z.theme?Z.theme[R.mode]:{},A={...R,colors:{...R.colors,...$}};return p.useImperativeHandle(X,()=>({goToDate:j=>{var Y;return(Y=f.current)==null?void 0:Y.goToDate(j)},goToToday:()=>{var j;return(j=f.current)==null?void 0:j.goToToday()},setZoom:j=>{var Y;return(Y=f.current)==null?void 0:Y.setZoom(j)}}),[]),p.useEffect(()=>{const j=()=>{E.current&&b(E.current.clientWidth)};return j(),window.addEventListener("resize",j),()=>window.removeEventListener("resize",j)},[]),E.current,a.jsxs(a.Fragment,{children:[a.jsx(rs,{}),a.jsx(es,{theme:A,children:a.jsx(Ti,{lang:Z.lang,translations:Z.translations,children:a.jsxs(Us,{data:n,isLoading:!!h,config:Z,onRangeChange:i,defaultStartDate:_,handleToggleDisplayActiveUnits:u,onClearFilterData:c,children:[a.jsx(da,{showScroll:!!n.length,$footer:Z.showOverview!==!1&&!!n.length,id:Ie,ref:E,children:a.jsx(ua,{children:a.jsx(fa,{data:n,baseData:r,categories:t,onTileClick:l,topBarWidth:m??0,onItemClick:d,toggleTheme:V,onEventDrop:y,onEventDrag:S,draggableConfig:x,schedulerRef:f,onTimeRangeSelect:k,onMultiTimeRangeSelect:g,clickToAddConfig:F})})}),Z.showOverview!==!1&&!!n.length&&a.jsx(la,{})]})})})]})}),pa=w.div`
  padding: 4px 11px 0;
  width: 100%;
...
`,Cc=34,Mc={sin_chofer:{stripe:"#9AA4B2",icon:"warn",color:"#9AA4B2"},sin_avisar:{stripe:"#D98A22",icon:"warn",color:"#D98A22"},notificado:{stripe:"#2C6BB0",icon:"clock",color:"#2C6BB0"},confirmado:{stripe:"#2E8B63",icon:"check",color:"#2E8B63"}},kc="#3E8E5A",$c="#D98A22",Dc=({row:e,data:n,zoom:t,isSubcontract:r=!1,onTileClick:o,onDragStart:s,isDragging:i=!1,isDraggable:l=!0,yOffset:u=0})=>{const{date:c}=Le(),d=fn(c,t),{y:h,x:y,width:S}=hc(e,d.startDate,d.endDate,n.startDate,n.endDate,t),{colors:x}=Et(),k=p.useRef(null),g=D(n.startDate).isSame(D(n.endDate),"day"),F=n.eventType===yt.Tour,X=n.eventType===yt.Transfer,Z=g&&(F||X),E=V=>{k.current={x:V.clientX,y:V.clientY},l&&s&&(V.preventDefault(),s(n,V))},f=V=>{if(k.current){const R=Math.abs(V.clientX-k.current.x),$=Math.abs(V.clientY-k.current.y);Math.sqrt(R*R+$*$)<=5&&(o==null||o(n)),k.current=null}else o==null||o(n)},m={left:`${y}px`,top:`${h+u}px`,backgroundColor:`${n.bgColor??x.defaultTile}`,width:`${S}px`,color:Or(n.bgColor??"")},b=!r&&n.readiness?Mc[n.readiness]:null,_=r&&n.subcontractConfirmed===!1,P=r?_?$c:kc:b==null?void 0:b.stripe,z=V=>a.jsxs(pc,{"data-segment-id":n.segmentId,style:m,onClick:f,onMouseDown:E,onDragStart:R=>R.preventDefault(),isDraggable:l,isDragging:i,$unconfirmed:_,children:[P&&a.jsx(gc,{style:{background:P}}),V]});return z(Z?a.jsxs(a.Fragment,{children:[(r||b)&&a.jsx(Ir,{$sm:!0,children:r?a.jsx(Nr,{children:"SUB"}):b&&a.jsx(Lr,{$sm:!0,style:{color:b.color},children:a.jsx(He,{name:b.icon,strokeWidth:b.icon==="check"?2.6:2.2})})}),a.jsxs(Sc,{children:[a.jsx(He,{name:X?"transfer":"sun",strokeWidth:2.4}),S>=Cc&&a.jsxs(a.Fragment,{children:[a.jsx(Fr,{children:D(n.startDate).format("HH:mm")}),!X&&a.jsx(Fr,{$end:!0,children:D(n.endDate).format("HH:mm")})]})]})]}):a.jsxs(a.Fragment,{children:[a.jsx(Ir,{children:r?a.jsx(Nr,{children:"SUB"}):b&&a.jsx(Lr,{style:{color:b.color},children:a.jsx(He,{name:b.icon,strokeWidth:b.icon==="check"?2.6:2.2})})}),a.jsxs(mc,{children:[a.jsxs(Yr,{$pad:!0,children:[a.jsx(yc,{children:a.jsx(He,{name:X?"transfer":"tour"})}),a.jsx(vc,{children:n.title})]}),a.jsxs(Yr,{children:[n.bookingNumber&&a.jsx(bc,{children:n.bookingNumber}),n.subtitle&&a.jsx(xc,{children:n.subtitle})]}),n.driver&&a.jsxs(wc,{children:[a.jsx(He,{name:"person"}),n.driver]})]})]}))},Hr=(e,n)=>{let t=0;for(const r of n)e>=r&&t++;return t*De},Ec=w.div`
  position: absolute;
  left: 0;
  right: 0;
  height: ${de}px;
  ...
  color: #93b1a6;
  pointer-events: none;
`,_c=({data:e,zoom:n,onTileClick:t,onDragStart:r,isDraggable:o,draggingEventId:s,separatorRowIndices:i=[]})=>{const l=p.useCallback(()=>{let u=0;return e.map((c,d)=>{if(d>0&&(u+=Math.max(e[d-1].data.length,1)),!c.data.some(h=>h.length>0)){const h=Hr(u,i);return[a.jsx(Ec,{style:{top:`${u*de+h}px`},children:"Disponible"},`dispo-${c.id}`)]}return c.data.map((h,y)=>h.map(S=>{const x=s===S.segmentId,k=o?o(S):!1,g=y+u,F=Hr(g,i);return a.jsx(Dc,{row:g,data:S,zoom:n,isSubcontract:c.isSubcontract,onTileClick:t,onDragStart:r,isDragging:x,isDraggable:k,yOffset:F},S.segmentId)}))}).flat(2)},[e,t,n,r,o,s,i]);return a.jsx(a.Fragment,{children:l()})};w.div`
  box-sizing: border-box;
  font-family: ${Ae};
  padding: 0 0.5rem;
  height: 125px;
...
`,nl=({draggedEvent:e,ghostPosition:n,ghostDimensions:t,dropTarget:r,isValidDrop:o,dragState:s,data:i,resourceOnly:l,separatorRowIndices:u=[]})=>{const c=Ne(),d=E=>{let f=0;for(const m of u)m<=E&&f++;return E*de+f*De},[h,y]=p.useState(null),[S,x]=p.useState(0),k=p.useCallback((E=400,f=300)=>{const b=t.width,_=48,P=document.getElementById("react-scheduler");if(!P)return{x:n.x+b+16,y:n.y};const z=P.scrollLeft,V=P.scrollTop,R=P.clientWidth,$=P.clientHeight,A=n.x-z,T=n.y-V,j={left:Ee+16,right:R-16,top:16,bottom:$-16},Y=j.right-(A+b),J=A-j.left,ee=j.bottom-(T+_),se=T-j.top;let oe,B;return Y>=E+16?oe=A+b+16:J>=E+16?oe=A-E-16:Y>=J?(oe=A+b+16,oe+E>j.right&&(oe=j.right-E)):(oe=A-E-16,oe<j.left&&(oe=j.left)),ee>=f+16?B=T+_+16:se>=f+16?B=T-f-16:ee>=se?(B=T+_+16,B+f>j.bottom&&(B=j.bottom-f)):(B=T-f-16,B<j.top&&(B=j.top)),oe=Math.max(j.left,Math.min(oe,j.right-E)),B=Math.max(j.top,Math.min(B,j.bottom-f)),{x:oe+z,y:B+V}},[n.x,n.y,t.width]);p.useEffect(()=>{s==="dragging"&&e&&S===0?x(n.x):s==="idle"&&x(0)},[s,e,n.x,S]),p.useEffect(()=>{y(s==="animating"&&e?{x:0,y:0}:null)},[s,e]);const g=p.useMemo(()=>{if(!e||!e.totalPassengers||s==="idle"||s==="potential")return[];const E=[];let f=0;for(const m of i){const b=Math.max(m.data.length,1);if(m.capacity!==void 0&&e.totalPassengers>m.capacity)for(let _=0;_<b;_++)E.push(f+_);f+=b}return E},[e,i,s]);if(!e||s==="idle"||s==="potential")return null;const F=s==="animating",X=Or(e.bgColor??""),Z=()=>{if(!r)return"";const E=D(r.startDate).format("MMM D, HH:mm"),f=D(r.endDate).format("HH:mm");return`${E} - ${f}`};return a.jsxs(Gc,{children:[g.map(E=>a.jsx(el,{style:{top:`${d(E)}px`,height:`${de}px`}},E)),r&&s==="dragging"&&a.jsx(Rc,{$isValid:o,$hasConflict:r.hasConflict,style:{top:`${d(r.resourceIndex)}px`,height:`${de}px`}}),r&&s==="dragging"&&!l&&a.jsxs(a.Fragment,{children:[a.jsx(Jc,{$isValid:o,$hasConflict:r.hasConflict,style:{left:`${n.x}px`,top:`${d(r.resourceIndex)+(de-48)/2}px`,width:`${t.width}px`}}),a.jsx(Qc,{$isValid:o,$hasConflict:r.hasConflict,style:{left:`${n.x}px`,top:`${d(r.resourceIndex)+(de-48)/2}px`},children:Z()})]}),r&&s==="dragging"&&l&&a.jsx(tl,{$isValid:o,$hasConflict:r.hasConflict,style:{left:"0px",top:`${d(r.resourceIndex)}px`,height:`${de}px`}}),r&&o&&r.hasConflict&&r.conflicts&&r.conflicts.length>0&&s==="dragging"&&(()=>{const E=k(400,300);return a.jsxs(Kr,{style:{left:`${E.x}px`,top:`${E.y}px`},children:[a.jsxs(qr,{children:[a.jsx(Jr,{children:"!"}),r.conflicts.length," ",r.conflicts.length>1?c.conflicts.detectedPlural:c.conflicts.detected," ",c.conflicts.detectedSuffix]}),a.jsx(Qr,{children:r.conflicts.map((f,m)=>{const b=D(r.startDate).format("YYYY-MM-DD"),_=D(r.endDate).format("YYYY-MM-DD"),P=D(f.event.startDate).format("YYYY-MM-DD"),z=D(f.event.endDate).format("YYYY-MM-DD"),V=D(f.conflictStart).format("YYYY-MM-DD"),R=D(f.conflictEnd).format("YYYY-MM-DD"),$=b!==_,A=P!==z,T=V!==R,j=$?D(r.startDate).format("MMM D, h:mm A"):D(r.startDate).format("h:mm A"),Y=$?D(r.endDate).format("MMM D, h:mm A"):D(r.endDate).format("h:mm A"),J=A?D(f.event.startDate).format("MMM D, h:mm A"):D(f.event.startDate).format("h:mm A"),ee=A?D(f.event.endDate).format("MMM D, h:mm A"):D(f.event.endDate).format("h:mm A"),se=T?D(f.conflictStart).format("MMM D, h:mm A"):D(f.conflictStart).format("h:mm A"),oe=T?D(f.conflictEnd).format("MMM D, h:mm A"):D(f.conflictEnd).format("h:mm A"),B=T?"":D(f.conflictStart).format("MMM D"),G=r.startDate.getTime(),K=r.endDate.getTime(),U=f.event.startDate.getTime(),C=f.event.endDate.getTime(),O=G>=U&&G<C,v=K>U&&K<=C,L=G<=U&&K>=C,H=U<=G&&C>=K;let I=!1,M=!1,te=!1,N=!1,q="";return L||H?(I=!0,M=!0,te=!0,N=!0,q=`⚠️ ${c.conflicts.changeBoth}`):O&&v?(I=!0,M=!0,te=!0,N=!0,q=`⚠️ ${c.conflicts.changeBoth}`):O?(I=!0,N=!0,q=`⚠️ ${c.conflicts.changeStart}`):v&&(M=!0,te=!0,q=`⚠️ ${c.conflicts.changeEnd}`),a.jsxs($n,{children:[a.jsxs(Dn,{children:[c.conflicts.conflictsWith,": ",f.event.title,f.event.subtitle&&` - ${f.event.subtitle}`]}),a.jsxs(lt,{children:[a.jsx("strong",{children:e.title})," ",c.conflicts.movingTo,":"," ",I?a.jsx("span",{style:{color:"#D32F2F",fontWeight:700},children:j}):j," ",c.conflicts.to," ",M?a.jsx("span",{style:{color:"#D32F2F",fontWeight:700},children:Y}):Y]}),a.jsxs(lt,{children:[a.jsx("strong",{children:f.event.title})," ",c.conflicts.currentlyAt,":"," ",te?a.jsx("span",{style:{color:"#D32F2F",fontWeight:700},children:J}):J," ",c.conflicts.to," ",N?a.jsx("span",{style:{color:"#D32F2F",fontWeight:700},children:ee}):ee]}),a.jsxs(Rr,{children:[c.conflicts.conflictTime,": ",B&&`${B}, `,se," - ",oe]}),q&&a.jsx(lt,{style:{backgroundColor:"#FFEBEE",color:"#C62828",fontWeight:600,marginTop:"6px",border:"1px solid #EF5350"},children:q})]},m)})})]})})(),r&&o&&!r.hasConflict&&r.nearbyEvents&&r.nearbyEvents.length>0&&s==="dragging"&&(()=>{const E=k(400,400);return a.jsxs(Kr,{style:{left:`${E.x}px`,top:`${E.y}px`,borderColor:"#4CAF50"},children:[a.jsxs(qr,{style:{color:"#2E7D32"},children:[a.jsx(Jr,{style:{backgroundColor:"#4CAF50"},children:"✓"}),r.nearbyEvents.length," ",r.nearbyEvents.length>1?c.conflicts.nearbyEvents:c.conflicts.nearbyEvent]}),a.jsxs(Qr,{children:[(()=>{const f=r.nearbyEvents.some(P=>P.position==="before"),m=r.nearbyEvents.some(P=>P.position==="after"),b=D(r.startDate).format("h:mm A"),_=D(r.endDate).format("h:mm A");return a.jsxs($n,{style:{backgroundColor:"#F1F8E9",borderLeftColor:"#8BC34A"},children:[a.jsxs(Dn,{style:{color:"#33691E"},children:[c.conflicts.yourEvent,": ",e.title,e.subtitle&&` - ${e.subtitle}`]}),a.jsxs(lt,{style:{fontWeight:600},children:[D(r.startDate).format("MMM D"),":"," ",f?a.jsx("span",{style:{color:"#2E7D32",fontWeight:700},children:b}):b," ",c.conflicts.to," ",m?a.jsx("span",{style:{color:"#2E7D32",fontWeight:700},children:_}):_]}),a.jsx(lt,{style:{backgroundColor:"#DCEDC8",marginTop:"4px",fontSize:"10px",color:"#558B2F"},children:c.conflicts.sameDay})]})})(),r.nearbyEvents.map((f,m)=>{const b=D(f.event.startDate).format("YYYY-MM-DD"),_=D(f.event.endDate).format("YYYY-MM-DD"),P=b!==_,z=P?D(f.event.startDate).format("MMM D, h:mm A"):D(f.event.startDate).format("h:mm A"),V=P?D(f.event.endDate).format("MMM D, h:mm A"):D(f.event.endDate).format("h:mm A"),R=D(f.event.startDate).format("MMM D"),$=Math.floor(f.timeGap/(1e3*60*60)),A=Math.floor(f.timeGap%(1e3*60*60)/(1e3*60)),T=$>0?`${$}h ${A}m`:`${A}m`,j=f.position==="after",Y=f.position==="before";return a.jsxs($n,{style:{backgroundColor:"#E8F5E9",borderLeftColor:"#4CAF50"},children:[a.jsxs(Dn,{style:{color:"#1B5E20"},children:[f.event.title,f.event.subtitle&&` - ${f.event.subtitle}`]}),a.jsxs(lt,{children:[!P&&`${R}: `,j?a.jsx("span",{style:{color:"#2E7D32",fontWeight:700},children:z}):z," ",c.conflicts.to," ",Y?a.jsx("span",{style:{color:"#2E7D32",fontWeight:700},children:V}):V]}),a.jsxs(Rr,{style:{backgroundColor:"#C8E6C9",borderColor:"#4CAF50",color:"#1B5E20"},children:[T," ",f.position==="before"?c.conflicts.before:c.conflicts.after]})]},m)})]})]})})(),a.jsx(Xc,{$isAnimating:F,$animateToX:h==null?void 0:h.x,$animateToY:h==null?void 0:h.y,style:{left:F?`${(h==null?void 0:h.x)??0}px`:"0",top:F?`${(h==null?void 0:h.y)??0}px`:"0",transform:F?void 0:`translate3d(${l?S:n.x}px, ${n.y}px, 0)`,backgroundColor:e.bgColor??"rgb(114, 141, 226)",width:`${t.width}px`,color:X},children:a.jsx(Uc,{children:a.jsxs(qc,{children:[a.jsx(Ur,{$bold:!0,children:e.title}),e.subtitle&&a.jsx(Ur,{children:e.subtitle}),e.description&&a.jsx(Kc,{children:e.description})]})})})]})},rl=Dt`
  0% {
...
`,wl=({zoom:e,startDate:n})=>{const{cols:t}=Le(),r=p.useMemo(()=>{if(e===2)return null;const o=e===0?_e*7:ye,s=D().year(n.year).month(n.month).date(n.dayOfMonth).startOf("day"),i=D().startOf("day"),l=e===0?i.startOf("week").diff(s.startOf("week"),"week"):i.diff(s,"days");return l<0||l>=t?null:{x:l*o,width:o}},[e,n,t]);return r?a.jsx(bl,{style:{left:`${r.x}px`,width:`${r.width}px`},"aria-hidden":!0}):null},ed="";$e.Scheduler=ha,Object.defineProperty($e,Symbol.toStringTag,{value:"Module"})});
